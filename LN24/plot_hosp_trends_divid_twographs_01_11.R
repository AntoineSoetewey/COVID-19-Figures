...
map1 <- ggplot(map) +
  ...
  labs(fill = bquote(atop(NA, atop("Hospitalisations journalières (x100,000)", bold(.(period1)))))) +
  theme_void() +
  theme(
    # Change legend
...
map2 <- ggplot(map) +
  ...
  labs(fill = bquote(atop(NA, atop("Hospitalisations journalières (x100,000)", bold(.(period2)))))) +
  theme_void() +
  theme(
    # Change legend
    legend.position = c(0.18, 0.2),
...
# save plot
png(file = "Belgian_Hospitalizations_0111.png", width = 15 * 360, height = 7 * 360, units = "px", pointsize = 7, res = 300)
