...
# choose period
subdat <- subset(dat, DATE >= "2020-09-20")
...
fig_trends <- ggplot(
  ...
  geom_text(aes(x = as.Date("2020-09-28"), y = 10, label = "Nombre de cas qui fait réagir l'Allemagne"),
  ...
  geom_text(aes(x = as.Date("2020-10-04"), y = 10, label = "Nombre de cas qui fait réagir l'Irlande"),
    colour = "lightgrey", angle = 90, vjust = -0.5
  ) +
  geom_vline(
  ...
  annotate("rect",
    ymin = -Inf, ymax = 2000 / 11431406 * 100000,
  ...
  scale_y_continuous(breaks = seq(from = 0, to = max(subdat$TOTAL_IN_ICU_divid) + 2, by = 2), limits = c(0, max(subdat$TOTAL_IN_ICU_divid) + 2)) +
...
map <- map %>%
  ...
  mutate(
    class1 = cut(per1,
      breaks = c(0, 0.5, 1.0, 1.5, 3, 6, 99),
      include.lowest = TRUE,
      labels = c("[ 0.0, 0.5 ]", "] 0.5, 1.0 ]", "] 1.0, 1.5 ]", "] 1.5, 3.0 ]", "] 3.0, 6.0]", " > 6.0")
    ),
    class2 = cut(per2,
      breaks = c(0, 0.5, 1.0, 1.5, 3, 6, 99),
      include.lowest = TRUE,
...
# save plot
png(file = "Belgian_ICU_BEL_0111.png", width = 15 * 360, height = 7 * 360, units = "px", pointsize = 7, res = 300)
